...
For each target scenario, use a fresh seed/debug session. Actions inside one seed session are sequential; isolated scenario sessions may run concurrently only under the conditions in 2.3:
...
Loop 2.2 over the targeted scenarios, restarting the seed so every test starts from a clean page. Sequential execution is the default. Parallel generation is safe only when every scenario owns a separate test process, a unique generated CLI session name, and non-colliding test data; stop each scenario's process and session independently.
