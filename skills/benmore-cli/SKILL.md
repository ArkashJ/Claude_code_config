...
## Install and reach a live page

```bash
# macOS (primary)
brew install Benmore-Studio/benmore/benmore-cli

# Linux / CI (primary; checksum-verified)
curl -fsSL https://benmore.ai/install-cli.sh | sh

benmore login  # browser flow; headless flags/env are below
cd ~/Benmore
...
The success criterion is a reachable page at the printed HTTPS URL. `benmore
new crm` never relocates the project outside `./crm`.
...
## Recent - through the release-readiness stack (Jun 2026)
...
|---|---|
| **Api-step `timeout:`**: `with.timeout` accepts `570s` or bare seconds (default 30s; step-level `timeout:` wins); `if:`-gated steps keep timeout/retry/expect_rows/on_error (v2.7.211) | `flows` |
| **Agent usage + prompt-cache visibility**: local Claude/Codex numeric counters, explicit sync, account opt-out, 30-day run detail, measured provider cache counters, and no prompt/transcript/code/path upload. Sync uploads only the tail since the last clean run, skips apps the account no longer has (stale `.benmore/app` marker) with a warning instead of failing, and coalesces a session that spanned several apps (v2.7.212). | `agent-usage` |
| **Stable API reference context**: descriptive `benmore api` requests use ETags and replay exact cached bytes on 304; `--call` is never cached. | `api-reference-cache` |
| **Existing frontends**: Vite/Next static export/CRA build externally, copy only generated output into `static/`, preserve SPA navigation fallback and asset 404s. | `existing-frontends` |
| **Native CLI installation**: Homebrew on macOS; checksum-verifying curl installer on Linux/CI; the npm package is only a deprecated correction. | `cli-install` |
| **CLI contracts**: workspace `.` resolution, dev-aware `open`, and stable usage/auth/not-found/transport exits. | `cli-contracts` |
| **Empty queries**: `_query` returns `{rows: [], count: 0}` for zero matches, never `rows: null`. | `query` |
| **Machine discovery**: authenticated OpenAPI 3.1 is identical at `/api/_openapi`, `/openapi.json`, and `/api/openapi.json`; public docs are identical at `/llms.txt` and `/.well-known/llms.txt`. | `machine-discovery` |
| **Browser SDK + explicit types**: native apps keep `import 'bm'`; external frontends import zero-dependency ESM `@benmore/bm`; `benmore types [app] --out PATH` refreshes generated schema declarations without localhost. | `sdk` |
| **Headless CLI auth**: `login`/`signup`/`bootstrap` accept `--token` or `--email` + `--password`, plus `BENMORE_TOKEN` / `BENMORE_EMAIL` / `BENMORE_PASSWORD`. Flag values win; env tokens authenticate every command without replacing saved credentials; incomplete non-TTY auth exits immediately. | `cli-auth` |
| **Reliable edit-to-deploy hooks**: `benmore hooks` merges Claude Code Edit/Write validation + push hooks without replacing existing settings; `benmore sync-status --all` checks every pulled app and must exit 0 before completion. Codex/other agents push explicitly; no automatic Codex hooks are assumed. | `sync-status` |
| **`run: delete_upload`**: server-only private-object deletion after an ownership-scoped lookup; accepts only this app's canonical stored reference, is idempotent when missing, and aborts/rolls back active SQLite work on failure (object deletion itself is external) | `uploads` |
| **Operator email-pause recovery**: after support reviews and cleans a bouncing recipient list, `benmore email <app-subdomain> resume` clears only the pause flag; history and global suppressions remain | `email` |
| **`run: sms` + `sms:` hook**: text from a flow (`with: { to, body }`, `text:` aliases `body:`) → `steps.<id>.outputs.to`, or on a data event (`sms: { to, body }` with a `when:` guard — usually load-bearing, since every send costs money). Delivery goes through the same provider selection as `SendSMS`: explicit Twilio/webhook wins, hosted platform's shared number is the fallback. Both shapes were DOCUMENTED but never implemented, so apps silently sent nothing; an empty `to`/`body` after interpolation is now a hard error, and unrecognized hooks.yaml keys are rejected at write time rather than dropped in silence | `sms` |
| **`run: transcribe`**: audio→text in a flow with ZERO vendor/API key — shells out to host `ffmpeg` + `whisper-cli` (whisper.cpp), ships in every edition. `with: { file, model?, language? }` → `steps.<id>.outputs.text`. Local-file-first (app-relative `uploads/…` path, no HTTP hop; URLs via the SSRF-guarded fetch), one transcription per process (serialized), loud actionable error when the binaries or ggml model are missing (gate with `if:` for soft-skip). Needs host provisioning — see `docs/frontiers/transcription.md` | `transcribe` |
| **Multiple custom domains per app** (v2.7.205): `benmore domain <app> <domain>` now ADDS the domain alongside existing ones (up to 10 per app - e.g. `ceo.example.com` + `vc.example.com` serving the same app); `benmore domain <app> <domain> --verify` / `--remove` target that one domain, omit the domain to verify/remove all; `benmore domain <app>` lists them | - |
| **Robust Markdown**: `bm.markdown(text)` server-renders via the benmark engine (CommonMark + GFM + syntax highlighting + mermaid + video/audio/PDF/YouTube/Vimeo embeds + wikilinks + front matter), sanitized for innerHTML. `bm.markdownBatch(items)` for lists; `{{ body \| markdown }}` gotmpl pipe; `<Markdown>`/`<KnowledgeBase>` components auto-theme + hydrate diagrams | `markdown` |
| **Sync safety + feature packs**: `benmore sync-status` compares local source to the deployed manifest before push/pull/deploy/delete; `.benmore/remote.json` records the last pulled base. `benmore feature list/export/install/clone` and `benmore clone` use versioned `feature.yaml` packs, dry-run install by default, with curated kanban, notify-hub, and foundry starters | `sync-status`, `feature-packs` |
| **GitHub sync** (v2.7.202): mirror an app's full commit history to a GitHub repo you own (personal or org) - connect once via OAuth (dashboard → Version Control), `benmore github <app> link [owner/repo]`, then every commit auto-pushes (Live → `main`, Sandbox → `dev`). One-way; secrets/data never leave the platform; `benmore github <app>` shows status + last push errors | `github` |
| `benmore deploy` of an existing directory is trustworthy (v2.7.201): first-deploy creates the app with a BARE scaffold (no Notes demo files lingering next to your app → no phantom `/api/notes` 404s), and a per-file validator rejection is reported + counted while the REST of the files still ship (pre-fix the deploy died silently at the first rejected file and everything after it never left the laptop) | - |
| **Managed social login, Stripe Connect, managed DNS** (v2.7.200): "Sign in with Google/Microsoft" works with ZERO client-ID setup on every hosted app (platform OAuth broker; BYO creds still white-label); dashboard → Payments connects Stripe with test keys auto-landing in dev + live keys in prod (`benmore payments <app>`); dashboard → Settings → Managed DNS hosts the domain's zone and auto-creates routing/DKIM/ownership records (`benmore dns <app> add <domain>`, Pro) | `oauth`, `payments`, `dns` |
| **Platform SMS service**: zero-config transactional SMS (phone-OTP auth, `run: sms`) from a shared toll-free number - US/CA, plan quotas (5/day free, 200/day paid), per-recipient velocity caps; paid apps can request a DEDICATED number (carrier registration automated, days-to-weeks review) and origination switches automatically on approval. `benmore sms <app>` + dashboard → Email & SMS (v2.7.199) | `sms` |
| **Platform email service**: every hosted app sends email with ZERO config (flow/hook `run: email`, auth OTP/verify/reset) from `<app>@mail.benmore.ai`; verify the user's OWN domain to send from their brand (`benmore email-domain <app> add <dom>` → 3 DKIM CNAMEs → auto-verifies; dashboard → Email tab; `email_domains` MCP tool). Plan-based daily quotas, platform-wide bounce/complaint suppression, auto-pause circuit breaker. `benmore email <app>` shows status (v2.7.197) | `email` |
| browser_check: `width`/`height` viewport args (mobile/responsive checks - were silently ignored), framework chrome (cookie banner/testing banner/feedback fab) hidden in ALL modes + click targets centered so overlays can't eat trusted clicks, bigger record-mode time budget + `deadline_exceeded` flag (no more error cascades; `share` refused for incomplete runs), quoted numbers in actions (`"y":"800"`) parse (v2.7.195) | - |
...
| Encryption is env-safe end to end: platform `sql`/`sql --write` use each tenant's OWN key (SELECT * decrypts; backfills via sql --write are readable by the app), ENCRYPTION_KEY is auto-pinned into the per-app env store at provision (survives restarts), undecryptable values return `[unavailable: decryption failed]` instead of raw `enc:…` ciphertext. Also: `promote` no longer false-fails on "backup dir not writable", `delete-file --env prod` works, `probe` bodies cap at 48KB (rune-safe, `body_parsed` always complete) (v2.7.194) | `encryption` |
| Tiered row access: `read: owner_or_role:manager,admin` - row visible to its owner OR any holder of a listed role (whole group on group-keyed tables, else all rows). Replaces the old "write a secure read flow" workaround (Jun 2026) | `access` |
| Per-flow `rate_limit: "5/hour per ip"` on `on.request` - 429 + `Retry-After` before any step runs; scopes ip/user/email/group (Jun 2026) | `flows` |
| Flows fail loud on guarded 0-row writes: `expect_rows: ">0"` assertion + `steps.<id>.outputs.rows_affected` (Jun 2026) | `flows` |
| `benmore status` / `diff <app>` - read-only dev↔prod drift (file + schema) before promote; `benmore doctor <app>` - one PASS/WARN release-readiness report (drift + env-parity + payment-keys + money-cols + PII + secrets) (Jun 2026) | - |
| Visitor feedback on a LIVE app: `features.feedback: true` (survives `publish`) / `benmore feedback-widget <app> on`; `benmore feedback resolve\|reopen <id>` triage loop (Jun 2026) | - |
| Enterprise hardening: idempotent async job replay, framework/cloud serve validators, `benmore test`, transactional migrations, `DB_SYNCHRONOUS=FULL\|EXTRA`, and platform WAL shipping/PITR (`BENMORE_WAL_SHIP=1`, `benmore restore --pitr --as-of`) (v2.7.168) | `async-flows`, `environments` |
...
Claude Code hooks fire automatically once `benmore hooks` has installed them. Codex and other agents do not have automatic Benmore hooks: push each edit explicitly and finish with `benmore sync-status --all` exit 0.

**Test mode vs live.** When `features.testing: true` in `app.yaml`, the framework injects an amber banner + visitor feedback widget on every page. That's the dev posture. To "go live", flip `features.testing: false` and save - that single edit ships and removes the banner. To **keep collecting feedback on the live app**, set `features.feedback: true` (or `benmore feedback-widget <app> on`): the widget stays, without the banner, and `publish` won't strip it.
...
#    With installed Claude Code hooks, Edit/Write triggers validate-write + push.
...
- **Hosted CLI** - `cli-install`, `cli-auth`, `cli-contracts`, `sync-status`, `agent-usage`, `api-reference-cache`, `machine-discovery`, `existing-frontends`
...
```bash
benmore login                                # one-time browser auth
benmore login --token TOKEN                  # validate + save an existing token
benmore login --email EMAIL --password PASS  # headless login; env equivalents: BENMORE_EMAIL/BENMORE_PASSWORD
BENMORE_TOKEN="$BENMORE_TOKEN" benmore apps  # ephemeral CI auth; never replaces saved credentials
...
benmore usage --local [--json]              # local attributed Claude/Codex counters only
benmore usage sync                          # scan + upload bounded numeric aggregates now
benmore telemetry on|off|status             # account-level upload preference
...
```bash
benmore pull <app> [target-dir] [--env dev|prod] [--force]
                                                # download deployed source as a workspace cache; writes .benmore/remote.json
benmore sync-status [dir] [--app NAME] [--env dev|prod] [--json]
                                                # compare local files to the deployed manifest. exits 0 clean, 1 drift, 2 conflict, 3 setup/auth/remote error
benmore sync-status --all                       # check every pulled app before completion; require exit 0
benmore hooks [status|check]                    # install/status Claude hooks; check runs final workspace drift scan
benmore push <path> [<path>...] [--app NAME] [--force]
                                                # push one or MORE files; blocks stale remote overwrites unless --force
benmore validate-write <path> --content-from-stdin   # dry-run write; PreToolUse hook runs this
benmore delete-file <path> [--app NAME] [--force]
                                                # remove a file from the deployed app; blocks stale remote deletes unless --force
benmore focus [repo]                         # select a nearby repo as read-only context, then start codex/claude in the current repo
```
...
With installed Claude Code hooks, you rarely run these by hand: installed Claude Code hooks fire on every Edit/Write. Codex, Cursor, and other agents run `benmore push <file>` explicitly after edits. `benmore pull` is the one you'll run explicitly in either workflow: at the start of a session, or to refresh a stale cache. If another editor changed the deployed app after your last pull, `push`, `delete-file`, `deploy`, `pull`, and `sync` block conflicts by default. Run `benmore sync-status --json` to see the exact file statuses (`synced`, `local_changed`, `remote_changed`, `conflict`, `local_only`, `remote_only`, `deleted_local`, `deleted_remote`). Use `--force` only when the user has explicitly chosen to overwrite the deployed source; after a successful force/push/pull, `.benmore/remote.json` refreshes.

### Feature packs - clone reusable pieces

Feature packs are the supported clone path. They are versioned `feature.yaml` tarballs that copy **code and schema shape**, not data, uploads, secrets, env values, or generated `src/bm.d.ts`.

```bash
benmore feature list

# Clone a curated starter into an app. Always dry-run first.
benmore clone notify-hub --app vint-compliance --env dev --dry-run
benmore clone notify-hub --app vint-compliance --env dev --apply

# Foundry is also a curated pack.
benmore feature clone foundry --app vint-compliance --env dev --dry-run

# Clone selected components straight from another app or app directory.
benmore clone source-app vint-compliance \
  --paths static/notifications.tsx,static/preferences.tsx \
  --models Notification,NotificationPreference \
  --flows broadcast_notification \
  --env dev --dry-run

# Avoid route/model collisions in a mature target app.
benmore clone notify-hub --app vint-compliance --env dev \
  --route-prefix ops --model-prefix Ops --dry-run
benmore clone notify-hub --app vint-compliance --env dev \
  --route-map /notify-hub=/alerts \
  --model-map Notification=AlertNotification --dry-run

# Clone a whole app over the target source. Dry-run first; apply is explicit.
benmore clone source-app vint-compliance --env dev --replace --delete-missing --dry-run
benmore clone source-app vint-compliance --env dev --replace --delete-missing --apply
```

Install is conservative: dry-run by default for remote apps; file/model collisions block unless `--replace` is explicit; schema additions are additive only; required env var names are reported but values are never copied; pack payloads are size/file-count bounded; the destination app is git-snapshotted before apply and rolled back if apply fails. If the user says “clone Notify Hub” use the curated pack. If they say “pick components to clone,” run `benmore clone <source> <target> --paths ... --models ... --flows ... --dry-run`, adding `--route-prefix`/`--model-prefix` for broad namespacing or `--route-map FROM=TO`/`--model-map Old=New` for exact collision rewrites. Route prefixes move static files, rename `flows/*.yaml` files, and rewrite known custom API route strings; exact route maps rewrite static route files, single-route flow files, and route strings; model prefixes/maps rewrite Prisma model names plus inferred table-name tokens in text files/flows. Installed `static/*.tsx` files are feature modules: import/link them from the existing app shell, or add a host `static/*.html` page, when the user wants a clean navigable route. Feature install does not rewrite the target app's nav automatically. If they say “clone the whole app over,” dry-run with `--replace --delete-missing` before applying. Do not combine `--delete-missing` with selected components, prefixes, or maps.

`benmore focus` is optional. Normal Claude/Codex sessions do not use it. Run it only when you intentionally want to code in one repo while using one nearby repo as the only external context source. It scans the current repo's parent, sibling repos, and one-level child repos; with no argument it shows a numeric selector. It writes `.benmore/focus.json` in the coding repo, exports `BENMORE_FOCUS_WORK` / `BENMORE_FOCUS_CONTEXT` to the launched process, and starts `codex` (or `claude`) with a prompt that points at only the selected context repo. Use `--clear` to remove `.benmore/focus.json`, `--list` to show candidates, `--json` for scripts, `--print` to print the prompt without launching an agent, `--agent none` to persist the focus and print the prompt, and `--from DIR` when running it outside the coding repo.
...
benmore health <app>                         # row/session/audit snapshot
benmore status <app>                         # dev↔prod drift: file + schema diff, in_sync verdict (alias: benmore diff)
benmore doctor <app>                         # release-readiness: drift + env-parity + payment-keys + money-cols + PII + secrets, one PASS/WARN report
...
benmore feedback <app> resolve <id> [<id>...]             # mark resolved (auto-routes dev/prod; --env to force)
benmore feedback <app> reopen <id> [<id>...]              # mark open again
benmore feedback <app> status <id> open|in_progress|resolved|wont_fix
benmore feedback <app> comment <id> <text...>
benmore feedback <app> delete <id>
benmore feedback-widget <app> [on|off]                   # keep the widget on the LIVE app (features.feedback)
```

**Triage loop (works straight through the merged list):** `benmore feedback <app>` returns each item with its `id` + `env`; fix the code (auto-pushes via the save hook), then `benmore feedback <app> resolve <id>` marks it done. `resolve`/`reopen` write straight to the live `data.db` (no redeploy) and route each id to the instance it actually lives in — dev and prod have colliding ids, so an ambiguous id errors instead of touching the wrong row (pass `--env dev|prod` to force).

The widget shows whenever `features.testing: true` (dev) OR `features.feedback: true`. The latter is the opt-in for collecting feedback on a **published** app - `publish` only flips `features.testing`, so it survives going live (default off; reads are admin-only on a live app). `benmore feedback <app>` merges submissions from BOTH the dev and prod instances, each tagged with its `env` + `origin_host`.
...
# Transfer ownership (two-step: owner initiates, recipient accepts; old owner kept as editor)
benmore transfer-ownership <app> user@example.com   # owner: initiate (emails the recipient)
benmore transfers                                   # recipient: list transfers waiting for you
benmore transfer-ownership --accept                 # recipient: accept (resolves the app when there's one pending)
benmore transfer-ownership <app> --accept           # recipient: accept a specific one (name or subdomain both work)
benmore transfer-ownership <app> --cancel           # either party: cancel
```

> Accept resolves the exact **subdomain** (e.g. `myapp-a1b2c3`), never the
> display name — but `benmore transfers` and the emailed accept command hand you
> the exact string, and `--accept` fuzzy-matches a display name / partial
> subdomain when it's unambiguous, so you never have to guess it.

### GitHub sync (one-way mirror)

```bash
benmore github <app>                         # status: linked repo, per-env last push, errors
benmore github <app> connect                 # prints the browser URL for the OAuth connect (once per account)
benmore github <app> link [owner/repo]       # create-or-attach + start mirroring (--public, --no-create)
benmore github <app> push [--force]          # mirror now (--force overwrites a diverged remote)
benmore github <app> unlink                  # stop mirroring (the GitHub repo is NOT deleted)
```

Every platform commit auto-pushes ~15s after a burst of edits: Live → `main`,
Sandbox → `dev`. One-way - nothing is ever pulled from GitHub, and secrets
(env.yaml), data.db, and uploads/ never leave the platform. Full reference:
`api(at:"github")`.
...
benmore describe <app> <subject>             # surface info: tables (row/col counts), table <name>, flows, hooks, webhooks, jobs, auth
...
- **`actions`** (run in order, before `script`): `{do, selector?, text?, key?, ms?, y?}` where `do` = `click` · `dblclick` · `type` (types `text` into `selector`) · `fill` (set value) · `focus` · `key` (`Enter`/`Tab`/`Escape`/`ArrowUp/Down/Left/Right`/`Backspace`, or a single char) · `scroll` (centers `selector` in the viewport, or scrolls to absolute `y`) · `wait` (`ms`). These are TRUSTED events (real clicks/keystrokes), so they fire dblclick, focus, type-to-edit, etc. that synthetic JS clicks don't.
...
- **`width` / `height`** (v2.7.195): viewport in CSS px (default 1440×810, clamped 320-3840 / 480-2160). `width:390, height:844` ≈ iPhone portrait, `768×1024` ≈ tablet - the way to verify responsive/mobile layouts (previously these args were silently ignored).
- **Overlays can't eat your clicks** (v2.7.195): the framework's injected chrome (testing banner, cookie consent, feedback fab) is hidden in EVERY mode, and click targets are centered in the viewport first so a sticky header/footer never intercepts the trusted click.
- **`deadline_exceeded: true`** in the result means the run hit its time budget mid-flow: remaining actions were skipped (one clear error, not a cascade) and `share:true` is refused for that run - a public link to a half-dead replay reads as a working demo of nothing. Split long flows into multiple calls. Recording runs get a bigger base budget since the rrweb boot + snapshot cost real seconds on heavy pages.

Use it after building any interactive UI: drive your primary actions with `actions`, read the result with `script`, fix every console error + failed request, `screenshot:true` + Read the file when design matters, and `record:true` when the user wants to watch the agent exercise the app. It is bound to the dev sandbox and runs through the CLI/MCP surface.
...
### Set up a secret
```bash
benmore env <app> set STRIPE_SECRET=sk_live_... GOOGLE_MAPS_API_KEY=...
```
...
### Going live
```bash
benmore doctor <app>                         # PRE-FLIGHT: release-readiness report (run this first)
benmore publish <app>                        # flips features.testing → false in one shot
```
Run `benmore doctor <app>` first - it rolls up dev↔prod drift, env-parity, payment-key sanity (live key in dev / test key in prod), money-as-Float columns, PII-exposed-to-anon tables, and any hardcoded secrets into one PASS/WARN verdict. `benmore publish` is equivalent to editing `app.yaml` and saving; the amber banner and feedback widget disappear (unless `features.feedback: true` keeps the widget). To go back to dev posture: `benmore unpublish <app>`.
...
- If `auth.verify_email` or `auth.otp` is on: emails are sent via the platform email service (Amazon SES; SMTP off-platform). Disclose the provider.
...
- Hosted apps send with ZERO config via the platform SES service. If `EMAIL_FROM` is set, the domain must be verified (`benmore email <app>` shows status; `benmore email-domain <app> add <domain>` to verify).
...
7. **Email is zero-config on the platform** (SES service; default sender `<app>@mail.benmore.ai`). A custom `EMAIL_FROM` requires a verified domain (`benmore email-domain <app> add <domain>`). Off-platform: SMTP_* env vars. Set via `benmore env <app> set ...`, not by editing `env.yaml` and pushing it.
...
15. **Changes ship per-file via `benmore push`.** The installed Claude Code PostToolUse hook runs it on every Edit/Write; Codex, Cursor, and other agents run it explicitly after edits. There is no bulk deploy in this flow - edit, push, verify against the live URL.
...
21. **Tiered row access: use `read: owner_or_role:<roles>`** for "owner sees own, manager/admin sees more." A row is visible to its owner OR any caller holding a listed role (the whole group on group-keyed tables, otherwise all rows). See `api({at:'access'})` → owner_or_role. (Only reach for a secure-read FLOW when the branching is more complex than role membership.)
...
44. **`src/bm.d.ts` auto-regenerates on every push that touches schema.prisma / app.yaml / flows.yaml / workflows.yaml / hooks.yaml / flows/*.yaml / i18n/*.yaml.** (v2.7.22+) Don't edit it by hand - the next push will overwrite. The placeholder dropped at scaffold time only has the SDK surface; per-app types (Message, Post, AggregateName union, WorkflowMap, Role union, etc) land after the first schema push. Force a refresh with `benmore types [app] --out src/bm.d.ts --env dev` (omit app inside its workspace). The fingerprint header at the top of the file (`// schema: <hash>`) lets `benmore check` warn when the workspace copy drifts from the deployed schema.
...
47. **For chat / feed / live-doc patterns, reach for the SDK primitive first** (v2.7.35-v2.7.37+). Seven patterns that chat agents used to re-write from scratch are all SDK calls now. **Before writing the hand-rolled version**, check:
    ...
    - Render user-typed Markdown? → `bm.markdown(text)` (server-rendered and sanitized by benmark), or `bm.markdownBatch(items)` for a list. Don't pull in a separate client Markdown library.
...
Write what feels right. The validator's error includes the corrected shape inline. Read it, fix the file, and save. Installed Claude Code hooks push the edit; from Codex, Cursor, or another agent run `benmore push <file>` explicitly. Hit the live URL with curl. Check logs. Iterate.
