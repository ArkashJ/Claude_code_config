---
name: start
description: Open a session properly — preflight (git/gh/PATH), enumerate sources with counts, working rules (commit/push/checkpoint continuously, verify at source, volunteered status). Use at the start of any coding session.
---
...
`gh issue list --limit 20`, `gh auth status`. KNOWN ENVIRONMENT FACT: `gh` FAILS inside the
sandbox on this machine ("invalid token" / "error connecting to api.github.com") because the
sandbox blocks the network and the macOS keychain where the token lives. This is an artifact,
not bad credentials — run ALL `gh` commands with escalated (outside-sandbox) execution from the
first call. Never diagnose it, never ask the human for permission they already gave, never fall
back to manual pushes; git-over-SSH also works if escalation is unavailable.
Also `command -v node gh playwright-cli vcheck`
— this shell does NOT inherit the user's PATH; if a tool is missing, fix PATH once
(`/opt/homebrew/bin`, never a version-pinned Cellar path) rather than per-command shims. Also check the project board if one exists
...
repo's history stays at count-level. If the ground is genuinely unfamiliar, do a dedicated mapping pass first instead of crawling inline.
...
## 4. Delegate — parallelize when the harness supports it

If this environment supports subagents/parallel workers, fan exploration and well-specified
implementation out to them and keep the main thread for deciding and editing. If it does not,
keep exploration reads TERSE (head/grep, not whole files) — main-thread context is the scarcest
resource. Every delegated task includes the commit/push cadence from rule 3.
...
- Before the first mutating call to any external system this session: assert the target
  identity in the same shell call — `aws sts get-caller-identity`, `benmore whoami && benmore apps`
  (whoami alone reads a cached credential and lies: e7faa32e), `gh repo view --json nameWithOwner`
  (escalated, per the sandbox fact above).
...
## Codex-specific rules (each one cost real sessions; evidence in synthesis/2026-08-04-codex-vs-claude.md)

- **Conventions are not auto-loaded here.** If the repo has CLAUDE.md (or similar conventions
  file), READ IT NOW — and if AGENTS.md doesn't exist, create it as a one-line pointer to
  CLAUDE.md and commit it, so future Codex sessions load conventions automatically.
- **Every shell call is a fresh shell.** `cd`, `export`, `$!`, and `mktemp` outputs do not
  survive between calls. Name working dirs deterministically (`/private/tmp/<slug>-<date>`)
  in a single call; never let a generated path be the only record of itself.
- **Approvals are session state.** Append every granted approval to a running decision log
  (scratch file or PR comment) the moment it's given; grep it BEFORE asking any approval
  question. A rejected write is a transport/credential fact until proven a consent fact —
  check the token/remote before asking the human for permission they already gave.
- **Latest instruction wins.** When a new instruction conflicts with a standing one, follow
  the newer and say which rule you're dropping. When blocked by a standing prohibition, name
  the exact phrase that lifts it ("reply 'merge approved' to lift the no-merge rule").
- **Never re-issue a byte-identical command.** Before re-running, state what changed since
  the last run; if nothing changed, the previous result is the result.
