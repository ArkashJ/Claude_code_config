...
2. Append to the rolling "Checkpoint log" comment on the draft PR (create it if missing):
   what landed since the last checkpoint, what is in flight (agents, background work — and
   whether their work is pushed or would die with them), what's next, plus any un-captured
   `SURPRISE:` / `FALSIFIED:` lines from recent work while they're fresh.
